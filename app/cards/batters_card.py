...
class BattersCardRenderer:
    ...
    def render(self) -> Image.Image:
        ...
        # --- Title bar ---
        _tlogo_sz = title_h - 8
        _has_title_logo = (cfg.show_logos and is_team_scope(cfg.scope)
                           and bool(self.entries) and bool(self.working_dir))
        ...
        if _has_title_logo:
            tlogo = get_logo(self.entries[0].team_abbrev, _tlogo_sz, self.working_dir)
            if tlogo:
                gap = 8
    ...
    def _title_text(self, font, avail_w: int) -> str:
        ...
        if is_team_scope(scope) and self.entries:
            ...
            if simple:
                return f"{team_name} Best Batters"
        ...
        if simple:
            short  = f"{label_s} Best Batters"
            long   = f"{label_l} Best Batters"
            bbox = font.getbbox(long)
            if (bbox[2] - bbox[0]) <= avail_w:
                return long
            return short

...
from app.data.batters_api import (
    BATTER_SCOPE_OPTIONS, SORT_STAT_LABELS,
    fetch_batters_cached, filter_batters, sort_and_trim,
)
...
class BatterTab(ttk.Frame):
    ...
    def _build_controls(self, parent: ttk.Frame) -> None:
        ...
        self._width_var  = tk.DoubleVar(value=self.settings.batters_width_in)
        self._height_var = tk.DoubleVar(value=self.settings.batters_height_in)
        ...
        if self.settings.batters_use_global_size:
        ...
        self._scope_var = tk.StringVar(value=self.settings.batters_scope)
        ...
        self._sort_var = tk.StringVar(value=self.settings.batters_sort_stat)
        ...
        self._col_mode_var = tk.StringVar(value=self.settings.batters_column_mode)
        for val, label in [
                ("auto",     "Auto (suggested)"),
                ("reduced",  "Reduced  (AVG HR RBI OPS)"),
                ("standard", "Standard  (AVG HR RBI OBP SLG OPS)"),
                ("extended", "Extended  (+H AB BB SO SB)"),
        ]:
        ...
        self._show_ts_var = tk.BooleanVar(value=self.settings.batters_show_timestamp)
        ...
        self._show_badges_var = tk.BooleanVar(value=self.settings.batters_show_rank_badges)
        ...
        self._show_explainers_var = tk.BooleanVar(
            value=self.settings.batters_show_col_explainers)
        ...
        self._bg_var = tk.StringVar(value=self.settings.batters_bg_color)
        ...
        self._bg_swatch = tk.Label(bg_row, width=3, relief="sunken",
                                   background=self.settings.batters_bg_color)
    ...
    def _fetch_and_preview(self, force: bool = False) -> None:
        ...
        self._set_status("Fetching batting stats…", error=False)
        self._canvas.delete("all")
        self._canvas.create_text(
            THUMB_W // 2, THUMB_H // 2,
    ...
    def _do_fetch(self, force: bool = False) -> None:
        try:
            block, source = fetch_batters_cached(
    ...
    def _do_render(self, block, source: str = "live") -> None:
        ...
        try:
            ...
            entries = filter_batters(block, cfg.scope)
            trimmed = sort_and_trim(entries, cfg.sort_stat, cfg.top_n, cfg.min_pa)

            if not trimmed:
                self._on_fetch_error(
                    f"No players found for scope '{cfg.scope}' with "
                    f"≥{cfg.min_pa} PA.")
                return

            renderer = BattersCardRenderer(cfg, trimmed, block,
                                           working_dir=self.settings.working_dir)
    ...
    def _build_card_config(self) -> BattersCardConfig:
        try:
    ...
    def _full_preview(self) -> None:
        ...
        win.title("Batters Card — Full Preview")
        img = self._card_image
        max_w = 1200
    ...
    # ------------------------------------------------------------------
    def apply(self) -> None:
        self.settings.batters_scope           = self._scope_var.get()
        self.settings.batters_column_mode     = self._col_mode_var.get()
        self.settings.batters_sort_stat       = self._sort_var.get()
        try:
            self.settings.batters_top_n       = int(self._topn_var.get())
        except (tk.TclError, ValueError):
            pass
        try:
            self.settings.batters_min_pa      = int(self._minpa_var.get())
        except (tk.TclError, ValueError):
            pass
        self.settings.batters_show_timestamp  = self._show_ts_var.get()
        ...
        self.settings.batters_show_col_explainers = self._show_explainers_var.get()
